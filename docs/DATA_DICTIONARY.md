...
- `cross_context_probe.csv` (9 rows): `ctx_a`, `ctx_b`, `n_compounds`,
  ...
  probe.
- `fold0_baseline_comparison.csv` (72 rows): `context`, `model`, `split`,
...
- `fold0_reference_model_summary.csv` (8 rows): one row per context with
...
Read `annex_hypotheses/README.md` and `HOW_TO_READ.md` first. All tables are
...
- `ensemble_rescoring_panel.csv` (43 rows): `public_compound_id`,
...
Read `annex_clusters/README.md` first. One table:

- `cluster_census.csv` (1,007 rows): `context`, `cluster_id`,
  `n_members`, `coherence`, `coherence_z`, `coherence_q`, `anchor_bb`,
  `anchor_pos`, `anchor_frac`, `top_up_set`, `top_up_q`, `top_dn_set`,
  `top_dn_q`, `prior_series_n`. One row per unsupervised analog-family
